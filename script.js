...
function mountDashboardChart(canvasId, config) {
...
function scheduleDashboardChartResize(delay = 140) {
...
function initDashboard() {
  ...
  scheduleDashboardChartResize(180);
...
function renderSalesChart(tickets) {
  ...
  mountDashboardChart('salesChart', {
    type: 'bar',
    data: {
      labels,
      datasets: [{
...
function renderCategoryChart() {
  ...
  mountDashboardChart('categoryChart', {
    type: 'doughnut',
    data: {
      labels: categories,
      datasets: [{
...
function renderLineChart() {
  ...
  mountDashboardChart('lineChart', {
    ...
    options: {
      ...
      plugins: {
        legend: {
          labels: { color: '#ffffff', font: { size: 11 }, boxWidth: 14 }
        }
      },
      scales: {
        x: {
...
function renderRevenuePieChart() {
  ...
  mountDashboardChart('pieChart', {
    type: 'pie',
    data: {
      labels: ['Music', 'Technology', 'Sports', 'Food & Drink', 'Arts', 'Entertainment'],
      datasets: [{
        data: [35, 22, 18, 10, 8, 7],
        backgroundColor: ['#a78bfa', '#ec4899', '#60a5fa', '#4ade80', '#fb923c', '#facc15'],
    ...
    options: {
      ...
      plugins: {
        legend: {
          position: 'bottom',
          labels: { color: '#ffffff', font: { size: 10 }, boxWidth: 12, padding: 10 }
        },
        tooltip: {
          callbacks: {
...
function renderRevenueBarChart() {
  ...
  mountDashboardChart('revenueBarChart', {
    type: 'bar',
    data: {
      labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'],
      datasets: [{
        label: 'Revenue (RM)',
        data: [8200, 12400, 9800, 18600, 22300, 28900],
        ...
        label: 'Target (RM)',
        data: [10000, 14000, 12000, 20000, 25000, 30000],
...
function renderTicketsTable() {
  ...
  tbody.innerHTML = sorted.map((t) => {
    ...
    return `
      <tr>
        <td class="ticket-id" data-label="Ticket ID">${t.id}</td>
        <td class="ticket-event-cell" data-label="Event">
        ...
        <td data-label="Seat"><span class="seat-badge">${t.seat}</span></td>
        <td data-label="Price">RM ${t.price}</td>
        <td data-label="Status"><span class="status-pill ${statusClass}">${t.status}</span></td>
        <td class="ticket-action-cell" data-label="Action">
          ${t.status === 'Confirmed'
